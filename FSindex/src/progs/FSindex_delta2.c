...
int main(int argc, char **argv)
{
  ...
  Try {
	...
	*cp = '\0';
	...
	/* Search using quasi-metric */
	HL = FSINDX_rng_srch(I, &query, D, rng[j], QUASI, HL, sfunc_type[i], pfunc_type[i]);

	/* Find delta - smallest metric radius such that metric ball includes 
	   the quasi-metric one */
	delta = 0;
	for (kk=0; kk < HL->actual_seqs_hits; kk++) 
	  delta = max(delta, MD->eval_score(MD, query.start, 
					    HL->hits[kk].subject.start, query.len));
	

	/* Search using metric */
	HL = FSINDX_rng_srch(I, &query, MD, delta, MAX, HL, sfunc_type[i], pfunc_type[i]);
