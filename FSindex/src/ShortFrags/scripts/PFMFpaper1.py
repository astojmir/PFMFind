...
if __name__=='__main__':
    ...
    for yeast_seq, domains in true_results(input_filename):
        ...
        for frag, num_hits in search_list:
            matching_domains = get_matching_domains(domains, length, frag)
            
            if len(matching_domains) > 0:
                histogram, tkw = get_domain_histogram(eid, length, frag)
                
                # Get the total frequencies of matching domains
                tmd = 0
                for dom in matching_domains:
                    if dom in histogram:
                        tmd += histogram[dom]

                # Now find the maximal adjusted relative frequency of
                # a matching domain
                max_rf = 0
                max_dom = 'NONE'
                max_count = 0
                for dom in matching_domains:
                    if dom not in histogram: continue
                        
                    rf = 1.0 * histogram[dom] / (tkw - tmd + histogram[dom])
                    if rf > max_rf:
                        max_dom = dom
                        max_rf = rf
                        max_count = histogram[max_dom]

                # Output the result - only those searches that match a
                # 'known' domain
                result =  (yeast_seq,
                           eid,
                           frag,
                ...
                ofp.write("%s\t%d\t%d\t%s\t%d\t%d\t%.3f\t%d\t%d\n" % result)
                ofp.flush()
                
#        print yeast_seq, len(search_list), c
